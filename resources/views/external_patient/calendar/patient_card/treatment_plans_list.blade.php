					...
					<table id="plan_table" class="table table-bordered table-striped  data-table nowrap" style="width:100%;">
									...
									<tbody>
									  ...
									  @foreach($treatment_plan as $p)
									   <tr>
									     <td><a onclick="PopupCenter('{{route('emr.visit.treatment_plans.index',[app()->getLocale(),$p->id])}} ', '{{__('Treatment Plan')}}','1000','500')" class="btn btn-action btn-sm">{{__('View')}}<span class="ml-1"><i class="fa fa-eye"></i></span></a>
										 <td>{{$p->id}}</td>
