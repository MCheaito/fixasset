				...
				<div class="card-body p-0">
				    <div  class="row m-1"> 
					   ...
					   <div class="col-md-6">
							...
							<input type="checkbox" id="freeCmd1" class="toggle-chk"/>
...
h.innerHTML=document.getElementById("invoice_nblot").value;
...
l.innerHTML=document.getElementById("formula_id").value;

o.innerHTML=document.getElementById("sel_price").value;
...
var ii=r.parentNode.parentNode.rowIndex;
 ...
 var  taxable=document.getElementById("myTable").rows[ii].cells[9].innerHTML;
...
var fclient=$('#selectclient').val();
		  if((fprof=='') && (fclient=='')){
		  Swal.fire({text:"{{__('The Supplier or client field is required')}}",icon:"error",customClass:"w-auto"});
		  return false;
		 }			 
...
let table = document.getElementById("myTable");
 ...
 arr.push({
			...
			FORMULAID:row.cells[11].innerHTML,
			SELPRICE:row.cells[12].innerHTML,
			INITPRICE:row.cells[13].innerHTML
        });	
...
var myjson=JSON.stringify(arr);
 $.ajax({
    type:'POST',
    data:{"_token": "{{ csrf_token() }}",data:myjson,"is_free":is_free,
	      "selcode":selcode,"selectpro":selectpro,"selectclient":selectclient,"valgst":valgst,"valqst":valqst,
