				  ...
				  <div class="col-md-2">
							   <div class="form-check">					
								   <label for="selecttax" class="mt-1 form-check-label"><b>{{__("Credit Note")}}</b></label>					
									<input id="cnote" class="ml-2 form-check-input" type="checkbox"  style="height:22px;width:22px;" name="cnote"/>
								</div> 	
				            </div> 				
				  <div class="col-md-2">
							   <div class="form-check">					
								   <label for="selecttax" class="mt-1 form-check-label"><b>{{__("Garanty")}}</b></label>					
									<input id="garanty" class="ml-2 form-check-input" type="checkbox"  style="height:22px;width:22px;" name="garanty"/>
								</div> 	
				            </div> 							
					   <div class="col-md-4">
					...
					<div  class="row m-1">  
						...
						<div class="col-md-3 col-6">
							<label for="selectcode"><b>{{__('Code')}}</b><span class="ml-1" id="NbItemStock" style="color:red;"></span></label>
					...
					<div class="col-md-3 col-6">
						...
						<div class="col-md-2 col-6">
							 ...
							 <input  type="text" class="form-control" name="invoice_sup"  value="" id="invoice_sup"  disabled /> 
						  </div>						
					  <div class="col-md-4">
...
$('#garanty').on('change', function()
{
$('#cnote').prop('checked', false);
});	

$('#cnote').on('change', function()
{
$('#garanty').prop('checked', false);
});
	...
	$.ajax({
		url: '{{route('GetInvoicesNb',app()->getLocale())}}',
		  ...
		  success: function(data){
			  $('#selectpro').val(data.supp);
			  $('#selectpro').trigger('change.select2');
	...
	$.ajax({
		url: '{{route('GetInvoicesDetails',app()->getLocale())}}',
          ...
          document.getElementById("totalf").value=data.total;
			...
			document.getElementById("invoice_sup").value=data.invoice_sup;
...
var garanty= $("#garanty:checked").val();

$("#valamountrefund").val("0.00");
$.ajax({
	url:'{{route("GetRefInventory",app()->getLocale())}}',
    type:'GET',
    data:{"_token": "{{ csrf_token() }}","invoice_id":invoice_id,"cnote":cnote,"garanty":garanty},
     success: function(data){
...
var result=document.getElementById("rowdelete"+ i).value;
	//}
if (result=="true"){    
arr[j]={"CODE":document.getElementById("myTable").rows[i].cells[0].innerHTML,"DESCRIP":document.getElementById("myTable").rows[i].cells[1].innerHTML,"QTY":retqty,"PRICE":document.getElementById("myTable").rows[i].cells[4].innerHTML,"DISCOUNT":document.getElementById("myTable").rows[i].cells[5].innerHTML,"TOTAL":document.getElementById("myTable").rows[i].cells[6].innerHTML,"TYPEDISCOUNT":document.getElementById("myTable").rows[i].cells[8].innerHTML,"TAXABLE":document.getElementById("myTable").rows[i].cells[9].innerHTML,"TVQ":document.getElementById("myTable").rows[i].cells[14].innerHTML,"TPS":document.getElementById("myTable").rows[i].cells[15].innerHTML,"EXPIRY_DATE":document.getElementById("invoice_date_val").value,"ID_DETAILS":document.getElementById("myTable").rows[i].cells[16].innerHTML,"FORMULAID":'0',"SELPRICE":'0.00',"INITPRICE":'0.00'};	
j++;
}
}
if(j==0){
   ...
   selectpro=document.getElementById("selectpro").value;
      selecttype="4";
	   ...
	   invoice_ref=$("#facture option:selected").text().trim();
	   	cnote= $("#cnote:checked").val();
		garanty= $("#garanty:checked").val();
			 comment=document.getElementById("comment").value;
			  invoice_sup=document.getElementById("invoice_sup").value;
   Mastatus='N';
   if (invoice_id!='')
   {
	 Mastatus='M';   
...
var myjson=JSON.stringify(arr);
 $.ajax({
    ...
    data:{"_token": "{{ csrf_token() }}",data:myjson,"invoice_date_due":invoice_date_due,
		  ...
		  "selecttax":selecttax,"totalf":totalf,"tdiscount":tdiscount,"selectpatient":selectpatient,"invoice_sup":invoice_sup,
		  "balance":balance,"status":Mastatus,"invoice_id":invoice_id,"reqID":reqID,"cnote":cnote,"garanty":garanty,
