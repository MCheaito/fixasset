			...
			<div class="card">	  
				...
				<div class="card-body p-0">
				    <div  class="row m-1"> 
					   ...
					   <div class="col-md-6">
					   	<label class="label-size">{{__("Free")}}</label>
							<label class="mt-2 slideon slideon-xs  slideon-success">
							<input type="checkbox" id="freeCmd1" class="toggle-chk"   {{isset($ReqPatient) && $ReqPatient->free=='Y'?'checked':''}}/>
					...
					<div  class="row m-1"> 
					  ...
					  <div class="form-group col-md-3 select2-primary">
											...
											<option value="0">{{__('Select Clients')}}</option>		
											@foreach($clients as $sclients)
											<option value="{{$sclients->id}}" {{isset($ReqPatient) && $sclients->id==$ReqPatient->client_id ? 'selected' : ''}}>{{$sclients->name}}</option>
											@endforeach 
											</select>
								...
								<table id="myTable" class="table table-striped table-bordered table-hover" style="text-align:center;">
									<thead>
										<tr class="txt-bg text-white text-center">
											...
											<th scope="col" style="font-size:16px;">{{__('Lot Nb')}}</th>
								   ...
								   <tbody>
									...
									@foreach ($ReqDeails as $sReqDeails) 
													...
													<td><input type="text" class="form-control" size="3"  id="valqty{{$cpt}}" value="{{$sReqDeails->qty}}"  onchange="RetRow(this,'valqty{{$cpt}}')"  oninput="RetRow(this,'valqty{{$cpt}}')" disabled /></td>											
...
//c.innerHTML=document.getElementById("valqty").value;
 ...
 c.innerHTML = '<input type="text" class="form-control" size="3" id="valqty' + 
                  document.getElementById("cpt").value + '" value="' + 
                  document.getElementById("valqty").value + 
                  '" oninput="RetRow(this, \'valqty' + document.getElementById("cpt").value + '\')" ' + 
                  'onchange="RetRow(this, \'valqty' + document.getElementById("cpt").value + '\')" />';
...
h.innerHTML=document.getElementById("nblot").value;

i.innerHTML=descriptypediscount;
j.innerHTML=taxablet;
k.innerHTML=document.getElementById("formula_id").value;
k.style.display = 'none';

l.innerHTML=document.getElementById("sel_price").value;
l.style.display = 'none';

m.innerHTML=document.getElementById("initprice").value;
m.style.display = 'none';

o.innerHTML='<input type="button" class="btn btn-delete" id="rowdelete'+document.getElementById("cpt").value+'" value="{{__("Delete")}}" onclick="deleteRow(this)"/>';
	...
	url: 'addInvoiceRow',
		...
		//	 $('#htmlTable').html(data.htmltable);
		   ...
		   //	$('#htmlTable').empty();
			...
			p.innerHTML=data.uqst;
			q.innerHTML=data.ugst;
			r.innerHTML="0";
			s.innerHTML="0";
...
var ii=r.parentNode.parentNode.rowIndex;
 ...
 var  valqty=parseFloat(document.getElementById("myTable").rows[ii].cells[2].innerHTML);
...
function saveinventory()
{
  ...
  let table = document.getElementById("myTable");
	...
	if (row.cells[2].getElementsByTagName("input").length > 0) {
            valqty = row.cells[2].getElementsByTagName("input")[0].value;
        }
 arr.push({
			...
			EXPIRY_DATE:row.cells[6].innerHTML,
			NBLOT:row.cells[7].innerHTML,

			TYPEDISCOUNT:row.cells[8].innerHTML,
			TAXABLE:row.cells[9].innerHTML,
			FORMULAID:row.cells[10].innerHTML,
			SELPRICE:row.cells[11].innerHTML,
			INITPRICE:row.cells[12].innerHTML,
			TVQ:row.cells[14].innerHTML,
			TPS:row.cells[15].innerHTML,
			RQTY:row.cells[16].innerHTML,
			RLID:row.cells[17].innerHTML,
        });	
...
var myjson=JSON.stringify(arr);
 $.ajax({
    ...
    data:{"_token": "{{ csrf_token() }}",data:myjson,
	      "tpay":tpay,"trefund":trefund,"stotal":stotal,"selectpro":selectpro,"selectclient":selectclient,"valgst":valgst,"valqst":valqst,
    ...
    success: function(data){
		if(data.success){	
          ...
          //disable button delete in table 
			...
			document.getElementById(valqty).setAttribute('disabled',false);
			}
			}
...
//alert(document.getElementById("cpt").rows[0].cells[1].innerHTML);
...
//	 disc =document.getElementById(s).value;
     valqty =document.getElementById("myTable").rows[i].cells[2].getElementsByTagName("input")[0].value;
	}		
...
function modifyinventory()
{
		...
		//	alert(rdel);
			...
			document.getElementById(valqty).disabled = false;
          //  document.getElementById(rdel).setAttribute('disabled',false);
			}
			}
