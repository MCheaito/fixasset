        ...
        <section class="col-md-12">	
			<div class="card">	  
				<div class="card-header">
				    ...
				    <div class="row">
					  <div class="col-md-9 col-7"><h5>{{__('Edit Adjustment')}} </h5></div>
							 ...
							 <input type="text" class="date_data form-control"  name="invoice_date_val" id="invoice_date_val" value="{{Carbon\Carbon::parse($ReqPatient->date_invoice)->format('Y-m-d H:i')}}" disabled />
										  ...
										  <div  class="col-md-2 col-6">
											...
											<select class="custom-select rounded-0" name="typeadjacement" id="typeadjacement">
													<option
														value="1" {{ $ReqPatient->typeadjacement == '1' ? 'selected' : '' }} >
															{{__('Add +')}}
													</option>
													<option
														value="2" {{ $ReqPatient->typeadjacement == '2' ? 'selected' : '' }} >
															{{__('Minus -')}}
													</option>
					...
					<label for="selecttax"><b>{{__("Tax")}}</b></label>					
						...
						@endphp
                       	<div class="col-md-6 col-8 {{$display}}">
											...
											<option value="{{$ReqPatientRets->id}}">{{$ReqPatientRets->clinic_inv_num}},{{__('Date')}}:{{$ReqPatientRets->datein}},{{__('total')}}:{{$ReqPatientRets->total}}</option>
											@endforeach 
											</select>
                                 </div>
						<div class="form-inline col-md-2 col-4 {{$display}}">
						  <button class="mt-4 btn btn-action" id="btnreturn" name="btnreturn" onClick="returnpatient()">{{__('View Return')}}</button> 
				...
				<div class="card-body">
								...
								<table id="myTable" class="table table-striped table-bordered table-hover table-sm" style="text-align:center;">
								   ...
								   <tbody>
									...
									@foreach ($ReqDeails as $sReqDeails) 
													...
													<td><input type="text"  class="form-control" size="5" id="discount{{$cpt}}" value="{{$sReqDeails->discount}}" onchange="discRow(this,'discount{{$cpt}}')" disabled /></td>
						...
						<div class="row mt-2 m-1">
									<div class="col-md-12 text-center">
									 @if(isset($ReqPatient->cmd_id))
									  <button class="m-1  btn btn-action" id="btnsave_cmd" name="btnsave_cmd" onClick="saveCMD()" disabled>{{__('Save')}}</button>
									 @else
...
$(document).ready(function(){
 ...
 //$("#cpt").val("0");
			...
			$("#typeadjacement").prop("disabled", true);
...
$('#barcode').on('change', function()
{
	...
	$.ajax({
		url:'{{route("fillBarcode",app()->getLocale())}}',
		   type: 'get',
		    data:{"_token": "{{ csrf_token() }}","barcode":current_val,inv_type:'other'},
		  dataType: 'json',
		  success: function(data){
			 ...
			 $("#selectcode").trigger('change.select2');
...
var q = $('#valqty').val(),sp=$('#sel_price').val(),dis=$('#valdiscount').val(); 
	 ...
	 trefund=document.getElementById("trefund").value;
      ...
      selecttype=document.getElementById("invoice_type").value;
	   ...
	   comment=document.getElementById("comment").value;
	    typeadjacement=document.getElementById("typeadjacement").value;
   Mastatus='N';
   if (invoice_id!='')
   {
	 Mastatus='M';   
...
var myjson=JSON.stringify(arr);
 $.ajax({
    type:'POST',
    data:{"_token": "{{ csrf_token() }}",data:myjson,"typeadjacement":typeadjacement,"invoice_date_due":invoice_date_due,
...
function modifyinventory()
{
	       ...
	       $("#typeadjacement").removeAttr("disabled", true);
			current_val=$("#selecttype").val();
...
function saveCMD(){
	...
	type:'POST',
    data:{"_token": "{{ csrf_token() }}","invoice_id":invoice_id,"invoice_date_val":$('#invoice_date_val').val()},
    dataType:'JSON',
	success: function(data){
...
e.innerHTML='<input type="button" class="btn btn-delete" id="rowdeleteref'+document.getElementById("cptr").value+'" value="{{__('Delete')}}" onclick="deleteRowRef(this)"/>';
...
var myjson1=JSON.stringify(arr1);
 $.ajax({
    type:'POST',
    data:{"_token": "{{ csrf_token() }}","selectpatient":selectpatient,"NreqIDRemis":NreqIDRemis,"invoice_id":invoice_id,"clinic_id":clinic_id,"invoice_date_val":invoice_date_val,data:myjson1},
...
for(ii=0;ii<document.getElementById("myTableRef").rows.length;ii++){
	//s = "test-"+i;
//arr[i]={"cell1":document.getElementById("myTable").rows[i].cells[0].innerHTML,"cell2":document.getElementById("myTable").rows[i].cells[1].innerHTML};	
arr1[ii]={"CODE":document.getElementById("myTableRef").rows[ii].cells[0].innerHTML,"DATE":document.getElementById("myTableRef").rows[ii].cells[1].innerHTML,"TYPE":document.getElementById("myTableRef").rows[ii].cells[2].innerHTML,"PRICE":document.getElementById("myTableRef").rows[ii].cells[3].innerHTML};	
	 ...
	 if(  (cmd !=null && cmd !='') || ref !=0 ){
		 (async () => {
		  ...
		  const inputOptions = new Promise((resolve) => {
				  setTimeout(() => {
					resolve({
					  'Payment': (type==2) ? '{{__("Payment receipt")}}' : '{{__("Garanty")}}',
					  'All': '{{__("All")}}',
