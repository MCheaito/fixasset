            ...
            <div class="row m-1">  
					...
					<h3>{{__('Clients')}}</h3>
								...
								<div class="mb-1 col-md-2">		
											...
											<a id="solItem" href="{{route('inventory.clients.clients',[app()->getLocale(),'1','0'])}}" class="btn btn-action  float-right">{{__('Add New')}}</a>
   ...
   <section class="col-md-12">
				    <div class="card"> 
						<div class="card-header text-white">
							 <div class="card-title"><h5>{{__("All clients")}}</h5></div>
...
$(function(){
	...
	var table = $('#clients_table').DataTable({
			...
			        scrollCollapse:true,
					ajax: {
						url: "{{ route('inventory.clients.index',app()->getLocale()) }}",
					    data: function (d) {
...
$('body').on('change','.toggle-chk',function(){
	...
	var id  = $(this).data("id");
	$.ajax({
            url: '{{route("inventory.clients.destroy",app()->getLocale())}}',
