					...
					<table id="dash_presc_table" class="table table-bordered table-striped  data-table nowrap" style="cursor:pointer;width:100%;">
									...
									<tbody>
									  @foreach($prescs as $p)
									   <tr>
									     <td>
										 {{$p->presc_id}}<a href="{{route('emr.visit.med_prescription.index',[app()->getLocale(),$p->presc_id])}}" target="_blank" class="ml-1 btn btn-icon btn-sm" title="{{__('View')}}"><i class="fa fa-eye" style="color:#1bbc9b"></i></a>
										 </td>
										 <td>{{$p->visit_num}}</td>
                                         ...
                                         <td>
										   @if(app()->getLocale()=='en')
										    @php 
										     $eye = isset($p->eye_type) && $p->eye_type !='None'?' , '.'Eye'.' : '.$p->eye_type:''; 
											 $date = isset($p->expiry_date) && $p->expiry_date !=''?' , '.$p->expiry_date:'';
											 $section = isset($p->section_en) && $p->section_en !='None'? '-'.$p->section_en:'';
											@endphp
											{{$p->title_en.$section.' , '.$p->voie_en.$eye.$date}}<br/>
											{{$p->dosage.' '.$p->dt_en.' '.$p->dp_en.' , '.$p->freq_en.' , '.$p->duration.' '.$p->duration_unit}} 
										   @endif
                                           @if(app()->getLocale()=='fr')
											@php 
										     $eye = isset($p->eye_type) && $p->eye_type !='None'?' , '.__('Eye').' : '.__($p->eye_type):''; 
											 $date = isset($p->expiry_date) && $p->expiry_date !=''?' , '.$p->expiry_date:'';
											 $section = isset($p->section_fr) && $p->section_fr !='Aucune'? '-'.$p->section_en.' , ':'';
											 @endphp
											{{$p->title_fr.$section.' , '.$p->voie_fr.$eye.$date}}<br/>
