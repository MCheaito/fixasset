<div class="container-fluid main-page">
     @if (count($errors) > 0)
        ...
        @endif
       	@php $display = isset($order) && $order->status!='V'?'':'d-none'; @endphp			
		<div  id="documents_upload" class="card  m-1 {{$display}}">
					...
					@if($documents->count())
					  @foreach($documents as $image)
						...
						<div class="text-center col-md-2 col-4 mb-2">
							...
							<button type="submit" id="destroy_attach" class="close-icon btn btn-icon btn-delete btn-sm" title="{{__('delete')}}" {{isset($order) && $order->status!='V'?'':'disabled'}}><i class="fa fa-times-circle"></i></button>
							</form>
						</div>
					 @endforeach
					@endif
