				   ...
				   <div class="card card-outline">
						...
						<div class="card-body p-0">
							...
							<div class="row m-1">			  
							   ...
							   <div class="col-md-1">					 
						   <button class="btn btn-action" id="btnpayment" name="btnpayment" onClick="paymentfacture()">{{__('Payments')}}</button>
				  ...
				  <!-- Modal content-->
    				<div class="modal-content">
						        ...
						        <div class="container-fluid">
									<div class="row">   
									  ...
									  <div class="col-md-3">	 
										  ...
										  <input type="text" class="form-control" name="date_pay" id="date_pay" value="{{Carbon\Carbon::now()->format('Y-m-d')}}" />
										</div>
									  </div> 
									   <div class="row">   
										 ...
										 <div class="col-md-2">
											   ...
											   <select class="form-control"   id="selectmethod" name="selectmethod">
													...
													<option value="S">{{__('Schedule')}}</option>
												</select>
										   </div>
										...
										<div class="col-md-1"  >
											<div class="d-flex flex-column">
											 ...
											 <button class="btn btn-action"  id="btnaddpay" onclick="insRowPayFacture()">{{__('Add')}}</button>
											</div>
										</div>		 
										</div>			
										<div class="row">  
													 ...
													 <table class="table-bordered table-sm" style="font-size:14px;">
													  <thead>
													   ...
													   <tbody>
														  ...
														  <td><input  class="billcss form-control"  value="" id="disamount"  disabled /></td>
						    ...
						    <div class="modal-footer justify-content-center" style="padding-top:2px;padding-bottom:1px;">
								 <button class="btn btn-action" id="btnsavepay" name="btnsavepay" onClick="event.preventDefault();savepaymentfacture()">{{__('Save')}}</button>
...
var bill_id=document.getElementById("filter_g").value;
$("#valamount").val("0.00");
$.ajax({
	url:'{{route("GetPaymentFacture",app()->getLocale())}}',
    type:'POST',
    data:{_token: '{{ csrf_token() }}',bill_id:bill_id,type:'P'},
     success: function(data){
	 $("#payamount").val(data.sumpay);
     $("#refamount").val(data.sumref);
	 $('#disamount').val(data.sumdis);
...
c.innerHTML=document.getElementById("selectmethod").value;
...
g.innerHTML='<input type="button" class="btn btn-delete" id="rowdeletepay'+document.getElementById("cptp").value+'" value="{{__('Delete')}}" onclick="deleteRowPayFacture(this)"/>';	
h.style.display="none";
h.innerHTML=$("#filter_g").val();

}
function deleteRowPayFacture(r){
 var ii=r.parentNode.parentNode.rowIndex;
 var  valpay=parseFloat(document.getElementById("myTablePay").rows[ii].cells[4].innerHTML);
 document.getElementById('myTablePay').deleteRow(ii);
}

function savepaymentfacture()
{
...
arr1[ii]={"CODE":document.getElementById("myTablePay").rows[ii].cells[0].innerHTML,
		  ...
		  "GUARANTOR":document.getElementById("myTablePay").rows[ii].cells[7].innerHTML,
		  "TYPE":document.getElementById("myTablePay").rows[ii].cells[2].innerHTML,
		  "CURRENCY":document.getElementById("myTablePay").rows[ii].cells[3].innerHTML,
		  "PRICE":document.getElementById("myTablePay").rows[ii].cells[4].innerHTML,
		  "RATE":document.getElementById("myTablePay").rows[ii].cells[5].innerHTML
		  };	
}
...
//var selectmethod=document.getElementById("selectmethod").value;	
var bill_id=document.getElementById("filter_g").value;
var bill_num=document.getElementById("reqIDPay").value;
...
var myjson1=JSON.stringify(arr1);
 $.ajax({
    type:'POST',
    data:{"_token": "{{ csrf_token() }}","bill_num":bill_num,"bill_id":bill_id,data:myjson1},
   url: '{{route("SavePaymentFacture",app()->getLocale())}}',
	success: function(data){
		if(data.success){	
			  Swal.fire({ 
              "text":data.success,
