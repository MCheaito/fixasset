...
<div class="container-fluid">
   <div class="card card-outline">	  
	  <div class="card-header card-menu">
					...
					<div class="row">
						<div class="col-md-12">
						  <h3 class="text-dark">Lab Tests Categories</h3>
					   </div> 
           ...
           <div class="container-fluid">  
			 ...
			 <div class="m-1 row">
			   <div class="col-md-12">
                            ...
                            <table id="cat_dt" class="table table-bordered table-striped data-table display compact nowrap" style="width:100%;">
                                <thead>
                                    <tr>
                                        <th class="all">#</th>
										<th>{{__('Order')}}</th>
...
@include('tests.categories.catLABModal')
@endsection
@section('scripts')
<script>
...
$(function(){
	...
	var table = $('#cat_dt').DataTable({
        ...
        ajax: {
            url: "{{ route('lab.tests_cat.index',app()->getLocale()) }}",
			data: function ( d ) {
	...
	$('#create_test').click(function () {
        ...
        $('#modelHeading').html("Create test category");
        $('#catLABModal').modal('show');
	...
	$('#saveBtn').click(function (e) {
        ...
        $.ajax({
          ...
          url: "{{ route('lab.tests_cat.store',app()->getLocale()) }}",
          type: "POST",
          dataType: 'json',
	...
	$('body').on('click', '.toggle-chk', function (e) {
		...
		Swal.fire({
		  ...
		  html:'{{__("Please note, this operation will affect the related groups for this category")}}',
		  showDenyButton: true,
		  ...
		  customClass: 'w-auto'
			  ...
			  if (result.isConfirmed) {
				  $.ajax({
					...
					type: "DELETE",
					url: "{{ route('lab.tests_cat.delete',app()->getLocale()) }}",
...
function editData(id) {
	  ...
	  $.ajax({
          data: {id:id},
          url: "{{ route('lab.tests_cat.get_info',app()->getLocale()) }}",
		  type: "POST",
          dataType: 'json',
		  ...
		  $('#modelHeading').html("{{__('Edit test category')}}");
