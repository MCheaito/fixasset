   ...
   <div class="card  p-1">
      <div class="card-header report-menu">
		 ...
		 <div class="row m-1">
			...
			<div class="col-md-2">
			   <label class="label-size">{{__('Has Request')}}</label>
			   <select class="form-control" name="filter_request" id="filter_request">
				 <option value="">{{__('Yes/No')}}</option>
				 <option value="R">{{__('Yes')}}</option>
...
$(document).ready(function(){
    ...
    ajaxURL:"{{route('reports.daily_invoices',app()->getLocale())}}", //ajax URL
    ajaxParams: function(){
		...
		var filter_request=$('#filter_request').val();
		return {
    ...
    columns:[
        {title:"{{__('Bill')}}", field:"bill_num"},
		{title:"{{__('Date')}}", field:"bill_date_time"},
		...
		{title:"{{__('Total')}}", field:"totalLBP",formatter:"money",formatterParams:{decimal:".",thousand:",",precision:0,},bottomCalc:"sum",bottomCalcParams: {precision: 0},bottomCalcFormatter:"money",bottomCalcFormatterParams:{decimal:".",thousand:",",precision:0,}},
		{title:"{{__('Remianing')}}", field:"soldLBP",formatter:"money",formatterParams:{decimal:".",thousand:",",precision:0,},bottomCalc:"sum",bottomCalcParams: {precision: 0},bottomCalcFormatter:"money",bottomCalcFormatterParams:{decimal:".",thousand:",",precision:0,}},
		{title:"{{__('Total$')}}", field:"totalUS",formatter:"money",formatterParams:{decimal:".",thousand:false,precision:2,},bottomCalc:"sum",bottomCalcParams: {precision: 2},bottomCalcFormatter:"money",bottomCalcFormatterParams:{decimal:".",thousand:false,precision:2,}},
		{title:"{{__('Remianing$')}}", field:"soldUS",formatter:"money",formatterParams:{decimal:".",thousand:false,precision:2,},bottomCalc:"sum",bottomCalcParams: {precision: 2},bottomCalcFormatter:"money",bottomCalcFormatterParams:{decimal:".",thousand:false,precision:2,}},
		],
    });
...
document.getElementById("download-pdf").addEventListener("click", function(){
    table.download("pdf", "lab_invoices.pdf", {
        orientation:"landscape", //set page orientation to portrait
        title:"{{__('Invoices')}}", //add title to report
    });
});	
...
document.getElementById("download-csv").addEventListener("click", function(){
    table.download("csv", "lab_invoices.csv");
});

//trigger download of data.xlsx file
document.getElementById("download-xlsx").addEventListener("click", function(){
    table.download("xlsx", "lab_invoices.xlsx", {sheetName:"{{__('Invoices')}}"});
});
...
$('#generate').click(function(e){
	...
	var filter_resource=$('#filter_resource').val();
	var filter_request=$('#filter_request').val();
	table.setData("{{route('reports.daily_invoices',app()->getLocale())}}", 
	              {filter_resource:filter_resource, filter_patient:filter_patient,
		           filter_fromdate:filter_fromdate,filter_todate:filter_todate,
				   filter_request:filter_request});
