...
<html>
  ...
  <body>
    ...
    @foreach($test_codes as $tc)
	     @php $codes = implode(',',$tc['codes']); @endphp
		 @if($cnt>=1)<div style="page-break-before:always;"></div>@endif
		<div style="width:100%;"> 
		       <!--<div style="font-size:11px;">{{__('File Nb.')}}&nbsp;:&nbsp;{{$patient_nb}}</div>-->
			    <div style="font-size:11px;">{{$patient_name}}</div> 
			    <div style="font-size:11px;">{{trim($patient_gender).$patient_age}} &nbsp; &nbsp;{{$coll_date}}</div>
			    @if($guarantor!="")<div style="font-size:11px;">{!!str_replace(' ','&nbsp;',$guarantor)!!}</div>@endif
			    @if($doctor_name!="")<div style="font-size:11px;">{!!str_replace(' ','&nbsp;',$doctor_name)!!}</div>@endif
			   <div style="font-size:11px;">{!!str_replace(' ','&nbsp;',$tc['specimen_name'])!!}&nbsp;:&nbsp;{{strlen($codes)>30?substr($codes,0,30).'...':$codes}}</div>
			   <div style="font-size:11px;">{{$order_id}}</div>
			   <div>{!! DNS1D::getBarcodeHTML($order_id, 'C128',2.5,44) !!}</div>
		 </div>
		 @php $cnt++; @endphp
     @endforeach  
  </body>
