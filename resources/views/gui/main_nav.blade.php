...
<nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav">
		 ...
		 @if($access_inventory || $access_billings)
		  <li class="d-none d-md-block nav-item dropdown navBarStyle" hidden >
				<a class="nav-link  dropdown-toggle" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" hidden> Invoices</a>
	   ...
	   @if($access_resources)
			<li class="d-none d-md-block nav-item dropdown navBarStyle">
				...
				<div class="dropdown-menu">	
					...
					<a class="dropdown-item" href="{{route('resources.index',app()->getLocale())}}" onclick="localStorage.clear();" hidden >{{ __('Doctors') }}</a>
                    <div class="dropdown-divider"></div>
					<a class="dropdown-item" href="{{route('external_labs.index',app()->getLocale())}}" hidden >{{ __('Guarantors') }}</a>
                    <div class="dropdown-divider"></div>
					<a class="dropdown-item" href="{{route('external_insurance.index',app()->getLocale())}}" hidden >{{ __('Referred Labs') }}</a>
                    <div class="dropdown-divider"></div>
					<a class="dropdown-item" href="{{route('prices.index',app()->getLocale())}}" onclick="localStorage.clear();" hidden>{{ __('Prices') }}</a>
		...
		@if($access_tests_settings)
			<li class="d-none d-md-block nav-item dropdown navBarStyle" hidden>
				<a class="nav-link dropdown-toggle" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" hidden>Tests Settings</a>
				<div class="dropdown-menu">	
	  ...
	  <li class="nav-item dropdown d-none d-md-inline-block navBarStyle">
        <a class="nav-link  dropdown-toggle" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{__('Reports')}}</a>
		  ...
		  @if($access_inventory)
				@switch(auth()->user()->type)
					@case(2)
						<a class="dropdown-item" href="{{ route('inventory.reports.index',app()->getLocale()) }}" onclick="localStorage.clear();" hidden>{{ __('Inventory') }}</a>
					@break
					@case(1)
					  @if(Session::has('inventory_branch_num'))
						   <a class="dropdown-item" href="{{ route('inventory.reports.index',app()->getLocale()) }}" onclick="localStorage.clear();" hidden>{{ __('Inventory') }}</a>
					  @else
						   <a class="dropdown-item" href="javascript:void(0)" onclick="localStorage.clear();Swal.fire({text:'{{__('Choose a branch')}}',icon:'error',customClass:'w-auto'});" hidden >{{ __('Inventory')}}</a>
					  @endif	
					@break
			  @endswitch
		   @endif
		   ...
		   @if($access_custom_reports)
				   <a class="dropdown-item" href="{{ route('custom_reports.index',app()->getLocale()) }}" hidden >{{ __('Template') }}</a>
		   @endif
          </div>    
       </li>
	  @endif
