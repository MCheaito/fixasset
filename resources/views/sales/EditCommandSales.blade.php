        ...
        <section class="col-md-12">	
			<div class="card">	  
				<div class="card-header">
				  <div class="row">
				  @if ($typein=='O')
					  <div class="col-md-9 col-7"><h5>{{__('Edit Order')}}</h5></div>
				  @else
					  <div class="col-md-9 col-7"><h5>{{__('Edit Visite')}}</h5></div>
				  @endif  
				...
				<div class="card-body p-0">
				    <div  class="row m-1"> 
						<div class="col-md-6"  >	
							<label class="label-size" hidden>{{__("Done")}}</label>
							<label class="mt-2 slideon slideon-xs  slideon-success" hidden>
					...
					@if($view_command_valid2)
						 @if(auth()->user()->type==2 ) 
							<label class="label-size">{{__("Facturation")}}</label>
							<label class="mt-2 slideon slideon-xs  slideon-success">
					...
					@endif
						 ...
						 <button class="m-1 float-right btn btn-action" id="btnpayment" name="btnpayment" onClick="paymentinvoice()" hidden >{{__('Pay')}}</button>
					...
					<div  class="row m-1"> 
						  ...
						  <div class="col-md-2">
							 <label for="reqID_val"><b>{{__('Order Nb')}}</b></label>
					    ...
					    <div class="col-md-2" >
							 ...
							 <input type="text" class="form-control"  name="invoice_date_due" id="invoice_date_due" value="{{Carbon\Carbon::parse($ReqPatient->date_due)->format('Y-m-d H:i')}}" disabled />
					  </div>
					<div class="col-md-2" >
							 <label for="invoice_date_due"><b>{{__('Delivery Date')}}</b></label>
							 <input type="text" class="form-control"  name="invoice_date_delivery" id="invoice_date_delivery" value="{{$ReqPatient->date_delivery}}" disabled />
					  </div>	
						<div class="col-md-2" >
							 <label for="invoice_date_due"><b>{{__('Ref.Fature')}}</b></label>
							 <input type="text" class="form-control"  name="ref_facture" id="ref_facture" value="{{$refinvoice}}" disabled />
					  </div>		
						  </div>
						<div  class="row m-1"> 
							
										<div class="form-group col-md-2 select2-primary">
											...
											<option value="{{$cl->id}}" {{isset($ReqPatient) && $cl->id==$ReqPatient->fournisseur_id ? 'selected' : ''}}>{{$cl->name}}</option>
											@endforeach 
											</select>
							...
							<div class="form-group col-md-2 select2-primary">
											...
											<option value="0">{{__('Choose a Client')}}</option>		
											@foreach($clients as $cl)
											<option value="{{$cl->id}}" {{isset($ReqPatient) && $cl->id==$ReqPatient->fournisseur_fid ? 'selected' : ''}}>{{$cl->name}}</option>
											@endforeach 
											</select>
					    ...
					    <div class="col-md-3">
							 <label for="invoice_rq"><b>{{__('Remark')}}</b></label>
 							  <textarea class="form-control" name="invoice_rq" id="invoice_rq" rows="4" disabled >{{$ReqPatient->notes}}</textarea> 
					  </div>
					   <div class="col-md-3">
							 <label for="newitem"><b>{{__('New Item')}}</b></label>
							  <textarea class="form-control" name="newitem" id="newitem" rows="4" disabled >{{$ReqPatient->newitem}}</textarea> 
					  </div>
						  </div>
					<div class="row m-1">
					<div class="form-group col-md-2 select2-teal">
						...
						<div class="form-group col-md-2 select2-teal">
										<label for="pro"><b>{{__('Analyzer').'*'}}</b></label>
											...
											<option value="0">{{__('Choose a Analyzer')}}</option>		
		    ...
		    <div class="card">
				<div class="card-body">
								...
								<table id="myTable" class="table table-striped table-bordered table-hover" style="text-align:center;">
								   ...
								   <tbody>
									...
									@foreach ($ReqDeails as $sReqDeails) 
													...
													<td>
													<select class="form-control" id="tpackage{{$cpt}}" disabled>
														<option value="U" {{ ($sReqDeails->tpackage === "U" ? 'selected' : '') }}>Unit</option> 
														<option value="R" {{ ($sReqDeails->tpackage === "R" ? 'selected' : '') }}>Rack</option> 
														<option value="B" {{ ($sReqDeails->tpackage === "B" ? 'selected' : '') }}>Box</option>
													</select>
													</td>
						...
						<div class="row mt-2 m-1">
								    ...
								    <button class="m-1 btn btn-delete" id="btndelete" name="btndelete" onClick="deleteinventory()" disabled >{{__('Delete')}}</button>
									@else
									<button class="m-1  btn btn-action" id="btnmodify" name="btnmodify" onClick="modifyinventory()">{{__('Modify')}}</button>
								    <button class="m-1 btn btn-delete" id="btndelete" name="btndelete" onClick="deleteinventory()">{{__('Delete')}}</button>
									@endif	
...
$(document).ready(function(){
 ...
 //$("#cpt").val("0");
			...
			$("#subselectpro").prop("disabled", true);	
...
$('#validateCmd2').change(function(e){
		...
		$.ajax({
			...
			data:{id:id,is_valid2:is_valid2,cleintid:cleintid},
			type: 'post',
			dataType: 'json',
			success: function(data){
				 // if(is_valid2=='Y'){
						$("#ref_facture").val(data.refinvoice);
						$("#invoice_date_delivery").val(data.datedelivery);  
			//	  }else{
			//	  }
				 Swal.fire({toast:true,title:data.msg,timer:1000,position:"bottom-right",icon:"success",showConfirmButton:false}).then(function(){
					 //window.location.href = data.location;
			//		 $("#ref_facture").val(data.refinvoice);
			//			$("#invoice_date_delivery").val(data.date_delivery);  
				 });
...
$('#quoteCmd1').change(function(e) {
    e.preventDefault();
    
    var id = $('#invoice_id').val();
    var cquote = $(this).is(':checked') ? 'Quote' : 'NotQuote';
    ...
    $.ajax({
        url: '{{route("CommandQuote", app()->getLocale())}}',
        data: { id: id, cquote: cquote },
        type: 'post',
        dataType: 'json',
        success: function(data) {
            Swal.fire({
                ...
                if (table) {
                    ...
                    for (let i = 1; i < rows.length; i++) {
                        ...
                        // Check if the 10th cell exists
                        if (cells.length > 11) {
                            const inputElement = cells[11].getElementsByTagName("input")[0];
                            if (inputElement) {
                                inputElement.value = (data.cquote === 'N') ? 'N' : 'Y';
                            }
                        }
                    }
                }
...
$('#selectcode').on('change', function()
{
   ...
   if(current_val=='0' || current_val==null){
	...
	$.ajax({
		url: '{{route("EfillPriceInvSales",app()->getLocale())}}',
...
g.innerHTML = '<input type="text" class="form-control" size="3"  id="rqty' + document.getElementById("cpt").value + '" value="0" oninput="RetRow(this, rqty' + document.getElementById("cpt").value + ')" onchange="RetRow(this, rqty' + document.getElementById("cpt").value + ')"/>';
...
h.innerHTML = '<input type="text" class="form-control" size="3"  id="rprice' + document.getElementById("cpt").value + '" value="' + document.getElementById("valprice").value + '" oninput="RetRow(this, rqty' + document.getElementById("cpt").value + ')" onchange="RetRow(this, rqty' + document.getElementById("cpt").value + ')"/>';

i.innerHTML="0";
l.innerHTML = '<input type="text" class="form-control" size="10"  id="rfacture' + document.getElementById("cpt").value + '" value="" />';
k.innerHTML = '<input type="text" class="form-control" size="2"  id="rpay' + document.getElementById("cpt").value + '" value="N" />';
...
function saveinventory() {
    ...
    for (let i = 0; i < table.rows.length; i++) {
        ...
        if (row.cells[6].getElementsByTagName("input").length > 0) {
            rqty = row.cells[6].getElementsByTagName("input")[0].value;
        }

        if (row.cells[7].getElementsByTagName("input").length > 0) {
            rprice = row.cells[7].getElementsByTagName("input")[0].value;
        }
       
		if (row.cells[2].getElementsByTagName("select").length > 0) {
		tpackage = row.cells[2].getElementsByTagName("select")[0].value;
		}
        if (row.cells[3].getElementsByTagName("input").length > 0) {
            qty = row.cells[3].getElementsByTagName("input")[0].value;
        }

        if (row.cells[4].getElementsByTagName("input").length > 0) {
            price = row.cells[4].getElementsByTagName("input")[0].value;
        }

        if (row.cells[9].getElementsByTagName("input").length > 0) {
            rfacture = row.cells[9].getElementsByTagName("input")[0].value;
        }

        if (row.cells[10].getElementsByTagName("input").length > 0) {
            rpay = row.cells[10].getElementsByTagName("input")[0].value;
        }
    ...
    if (answer) {
        ...
        let subselectpro = document.getElementById("subselectpro").value;
            ...
            success: function(data) {
                ...
                if (data.success) {
                    ...
                    for (let j = 1; j <= nb; j++) {
                        ...
                        let rpay = "rpay" + j;
						let tpackage = "tpackage" + j;
                        if (document.getElementById(tpackage) != null) {
                            document.getElementById(tpackage).setAttribute('disabled', true);
                        }
...
function modifyinventory()
{
		...
		$("#selectpro").removeAttr('disabled');	
		$("#subselectpro").removeAttr('disabled');	
		...
		//  	alert(nb);
			...
			var rfacture="rfacture"+j;
			var rpay="rpay"+j;
			var tpackage="tpackage"+j;

		//	alert(rdel);
			j++;
			document.getElementById(rprice).disabled = false;
			document.getElementById(tpackage).disabled = false;
...
var nb=document.getElementById("cpt").value;
 ...
 var  rvalqty=parseInt(document.getElementById("myTable").rows[ii].cells[6].getElementsByTagName('input')[0].value);
 var  rprice=parseInt(document.getElementById("myTable").rows[ii].cells[7].getElementsByTagName('input')[0].value);
 ...
 document.getElementById("myTable").rows[ii].cells[8].innerHTML=newtotal;
...
$('#selectfou').val('0').trigger('change');
 ...
 $('#selectcat,#selectmat,#selectana').on('change',function(){
	...
	$.ajax({
		  url: '{{route("fill_code_categoty",app()->getLocale())}}',
		  data:{"_token": "{{ csrf_token() }}","category":category,"analyzer":analyzer,"type":type,"filter":"2"},
		  type: 'get',
		  dataType: 'json',
		  success: function(data){
		  $('#select_code').empty();
...
function downloadPDF(){	 
 ...
 var refinvoice = $('#ref_facture').val();
 if (refinvoice!=''){
 $.ajax({
				   url: '{{route("salesdownloadPDFInvoice",app()->getLocale())}}',
				   beforeSend: function() { 
								 Swal.fire({
									title: '',
									html: '{{__("Please wait...")}}',
				   ...
				   data: {'_token': '{{ csrf_token() }}','id':refinvoice,'print_type':'All','desc':''},
				   type: 'post',
