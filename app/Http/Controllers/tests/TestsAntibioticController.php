...
class TestsAntibioticController extends Controller
{
    ...
    public function index($lang,Request $request)
    {
         $user_type = auth()->user()->type;
		  ...
		  $antibiotic = DB::table('tbl_lab_antibiotic')->where('active','Y')->orderBy('testord')->orderBy('id','desc')->get();
	  ...
	  if($request->ajax()){
		  $filter_status='';
		   ...
		   if(isset($request->filter_status) && $request->filter_status!=''){
               ...
               $data = DB::table('tbl_lab_antibiotic')
        ...
        return view('tests.antibiotic.index')->with(['my_labs'=>$my_labs]);
    }
   ...
   public function get_info($lang,Request $request)
    {
	    ...
	    $id = $request->id;
		$data = DB::table('tbl_lab_antibiotic')->find($id);
    ...
    public function store($lang,Request $request)
    {
	   ...
	   return response()->json(['success'=>__('Saved successfully')]);
		  ...
		  $c = DB::table('tbl_lab_antibiotic')->where('id','<>',$id)->where('code',$code)->first();
		  if(isset($c) && isset($c->code) && $c->code!=''){
			  return response()->json(['error'=>'Code already exists for another category']);
		  }
		   
		   
		   DB::table('tbl_lab_antibiotic')->where('id',$id)->update([
			 'code'=>$request->code,
			 'descrip'=>$request->descrip,
			 'testord'=>isset($request->testord)?$request->testord:0,
			 'user_num'=>$user_id
    ...
    public function destroy($lang,Request $request)
    {
		...
		if($checked=='N'){
		DB::table('tbl_lab_antibiotic')->where('id',$id)->update(['active'=>'N','user_num'=>auth()->user()->id]);
		...
		if($checked=='O'){
	    DB::table('tbl_lab_antibiotic')->where('id',$id)->update(['active'=>'Y','user_num'=>auth()->user()->id]);
