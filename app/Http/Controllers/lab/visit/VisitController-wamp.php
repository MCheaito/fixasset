...
private function getPDFPath($lang,$order_id){
	...
	$result_path =  "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\ResultPDF\\".$request_nb;
	...
	$pdfContent =  "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\ResultPDF\\".$new_path;
    return $pdfContent;  	
}
public function printResults($lang,Request $request){
	...
	if($order->status=='V' && isset($order->pdf_path) && $order->pdf_path!=''){
	    $pdfContent = "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\".$order->pdf_path; 
	}else{
		$pdf = $this->getPDF($lang,$order_id,$type);			
		//save PDF as file
		$result_path = "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\ResultPDF\\".$order->request_nb;
		if(!file_exists($result_path)){mkdir($result_path, 0775, true);}
		$files = glob($result_path.'\\*'); 
		foreach($files as $file){ if(is_file($file)) unlink($file);} 
		$result_file= uniqid().'.pdf';
		$result_file_path =  "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\ResultPDF\\".$order->request_nb."\\".$result_file;
		file_put_contents($result_file_path,$pdf);
		...
		$pdfContent =  "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\".$new_path;  
	} 
...
function getPDF($lang,$order_id,$type){
		   ...
		   $results = DB::table('tbl_visits_order_results as r')
			...
			$filename = uniqid() . '.pdf';
			$filepath =  "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\ResultPDF\\".$filename;
			...
			if($pdf_docs->count()>0 || $custom_reports->count()>0){
				...
				foreach($pdf_docs as $d){
						$pdf_path = "C:\\wamp64\\www\\source-emr-labalhadi\\storage\\app\\7mJ~33\\ResultPDF\\".$d->path;
